...
from pytorch_utils_evaluation import forward
import config
...
class Evaluator(object):
    ...
    def evaluate(self, data_type, source, max_iteration=None, verbose=False):
        ...
        labelsx = ['airport', 'shopping_mall', 'metro_station', 'street_pedestrian',
        ...
        for i in y_pred:
            print (i)
            print (labelsx[i])
            label.append(labelsx[i])

        dataframe = pd.DataFrame({'filename': audio_name, 'label': label})
