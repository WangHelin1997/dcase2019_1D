...
def forward(model, generate_func, cuda, return_input=False, 
    ...
    for batch_data_dict, batch_data_dict_left, batch_data_dict_right, batch_data_dict_side,\
            batch_data_dict_harmonic, batch_data_dict_percussive in generate_func:
        ...
        batch_feature_harmonic = move_data_to_gpu(batch_data_dict_harmonic['feature_harmonic'], cuda)
        batch_feature_percussive = move_data_to_gpu(batch_data_dict_percussive['feature_percussive'], cuda)
        
        with torch.no_grad():
            model.eval()
            batch_output = model(data=batch_feature, data_left=batch_feature_left, data_right=batch_feature_right,
                                 data_side=batch_feature_side, data_percussive=batch_feature_percussive,
                                 data_harmonic=batch_feature_harmonic)
        ...
        if return_input:
            ...
            append_to_dict(output_dict, 'feature_harmonic', batch_data_dict['feature_harmonic'])
            append_to_dict(output_dict, 'feature_percussive', batch_data_dict['feature_percussive'])
