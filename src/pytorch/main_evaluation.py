...
from data_generator_evaluation import DataGenerator
from models_plus import Cnn_9layers_AvgPooling_mix, Cnn_9layers_MaxPooling
from losses import nll_loss
from evaluate_evaluation import Evaluator, StatisticsContainer
from pytorch_utils_evaluation import move_data_to_gpu
import config
...
def inference_evaluation(args):
    '''Inference and calculate metrics on evaluation data.
    ...
    evaluation_csv = os.path.join(dataset_dir, sub_dir, 'evaluation_setup',
                                'test.csv')
    ...
    checkpoint_path = os.path.join('/home/cdd/DCASE2019/task_1/workspace', 'checkpoints', 'main_augment',
                                   '{}logmel_{}frames_{}melbins'.format(prefix, frames_per_second, mel_bins),
                                   'TAU-urban-acoustic-scenes-2019-development', 'holdout_fold={}'.format(holdout_fold),
                                   model_type, '{}_iterations.pth'.format(iteration))
    ...
    if subtask in ['a', 'c']:
        evaluator.evaluate(data_type='validate', source='a')
...
if __name__ == '__main__':
    ...
    parser_inference_validation.add_argument('--subtask', type=str, choices=['a', 'b', 'c'], required=True,
                                             help='Correspond to 3 subtasks in DCASE2019 Task1.')
    parser_inference_validation.add_argument('--data_type', type=str, choices=['evaluation'], required=True)
