...
def mel_frequencies(n_mels=128, fmin=0.0, fmax=21050.0, htk=False):
...
#
#     def __init__(self, classes_num=50, sr=44100, activation='logsoftmax'):
#         super(MyNet, self).__init__()
#         # 1: feature extraction
#         self.stft = STFT(filter_length=2048, hop_length=1024, win_length=None,
#                          window='hamming')
#         self.melspec = melspectrogram(sr=sr, n_fft=2048, n_mels=64, fmin=0.0, fmax=None, htk=True,
#                                       norm=1, dtype=np.float32, trained=False)
#         # 2: CNN
#
...
class Cnn_9layers_AvgPooling_mix(nn.Module):
    ...
    def __init__(self, classes_num, activation):
        ...
        self.activation = activation
        self.bn1 = nn.BatchNorm1d(441000)
        self.bn2 = nn.BatchNorm1d(441000)
        self.bn3 = nn.BatchNorm1d(441000)
        self.bn4 = nn.BatchNorm1d(441000)
        self.stft = STFT(filter_length=2048, hop_length=1024, win_length=None,
                         window='hamming')
        self.melspec = melspectrogram(sr=44100, n_fft=2048, n_mels=64, fmin=0.0, fmax=None, htk=True,
                                      norm=1, dtype=np.float32, trained=False)
        self.conv_block1 = ConvBlock_mix(in_channels=4, out_channels=64)
        self.conv_block2 = ConvBlock_mix(in_channels=64, out_channels=128)
        self.conv_block3 = ConvBlock_mix(in_channels=128, out_channels=256)
        self.conv_block4 = ConvBlock_mix(in_channels=256, out_channels=512)
        self.fc2 = nn.Linear(512, 512, bias=True)
    ...
    def forward(self, data, data_left, data_right, data_side):
        '''
        Input: (batch_size, times_steps, freq_bins)'''
