...
from utilities import (create_folder, read_audio, calculate_scalar_of_tensor, 
    pad_truncate_sequence, get_subdir, read_metadata, read_left_audio)
import config
...
def calculate_feature_for_all_audio_files(args):
    ...
    feature_path = os.path.join(workspace, 'features_left',
    ...
    for (n, audio_name) in enumerate(meta_dict['audio_name']):
        ...
        hf['feature_left'].resize((n + 1, total_samples))
        hf['feature_left'][n] = feature
...
def calculate_scalar(args):
    ...
    scalar_path = os.path.join(workspace, 'scalars_left',
        '{}logmel_{}frames_{}melbins'.format(prefix, frames_per_second, mel_bins), 
        '{}.h5'.format(sub_dir))
