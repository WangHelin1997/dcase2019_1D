...
from utilities import (create_folder, read_audio, calculate_scalar_of_tensor, 
    pad_truncate_sequence, get_subdir, read_metadata, read_right_audio)
import config
...
def calculate_feature_for_all_audio_files(args):
    ...
    feature_path = os.path.join(workspace, 'features_right',
    ...
    for (n, audio_name) in enumerate(meta_dict['audio_name']):
        ...
        feature = feature[0 : total_samples]
        
        hf['feature_right'].resize((n + 1, total_samples))
        hf['feature_right'][n] = feature
...
def calculate_scalar(args):
    ...
    scalar_path = os.path.join(workspace, 'scalars_right',
        '{}logmel_{}frames_{}melbins'.format(prefix, frames_per_second, mel_bins), 
        '{}.h5'.format(sub_dir))
