...
class LogMelExtractor(object):
    ...
    def transform(self, audio):
        '''Extract percussive feature of a singlechannel audio file.
        ...
        '''
        audio_h, audio_p = librosa.effects.hpss(audio)
...
def calculate_feature_for_all_audio_files(args):
    ...
    feature_path = os.path.join(workspace, 'features_percussive',
    ...
    for (n, audio_name) in enumerate(meta_dict['audio_name']):
        ...
        hf['feature_percussive'].resize((n + 1, frames_num, mel_bins))
        hf['feature_percussive'][n] = feature
...
def calculate_scalar(args):
    ...
    scalar_path = os.path.join(workspace, 'scalars_percussive',
        '{}logmel_{}frames_{}melbins'.format(prefix, frames_per_second, mel_bins), 
        '{}.h5'.format(sub_dir))
    ...
    with h5py.File(feature_path, 'r') as hf:
        features = hf['feature_percussive'][:]
