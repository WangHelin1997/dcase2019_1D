...
class LogMelExtractor(object):
    ...
    def transform(self, audio):
        '''Extract Harmonic feature of a singlechannel audio file.
...
def calculate_feature_for_all_audio_files(args):
    ...
    feature_path = os.path.join(workspace, 'features_harmonic',
    ...
    for (n, audio_name) in enumerate(meta_dict['audio_name']):
        ...
        hf['feature_harmonic'].resize((n + 1, frames_num, mel_bins))
        hf['feature_harmonic'][n] = feature
...
def calculate_scalar(args):
    ...
    scalar_path = os.path.join(workspace, 'scalars_harmonic',
        '{}logmel_{}frames_{}melbins'.format(prefix, frames_per_second, mel_bins), 
        '{}.h5'.format(sub_dir))
